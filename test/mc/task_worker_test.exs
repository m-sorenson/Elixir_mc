defmodule MC.TaskWorkerTest do
  ...
  test "valid states tests" do
	...
	test "unsafe states tests" do
    assert false == MC.TaskWorker.safe?({1, 1, 3})
		assert false == MC.TaskWorker.safe?({1, 1, 2})
		assert false == MC.TaskWorker.safe?({1, 1, 0})
    assert false == MC.TaskWorker.safe?({1, 2, 3})
		assert false == MC.TaskWorker.safe?({1, 2, 1})
		assert false == MC.TaskWorker.safe?({1, 2, 0})
  end

  test "safe states tests" do
    assert true == MC.TaskWorker.safe?({1, 0, 3})
    assert true == MC.TaskWorker.safe?({1, 0, 2})
    assert true == MC.TaskWorker.safe?({1, 0, 1})
		assert true == MC.TaskWorker.safe?({1, 0, 0})
    assert true == MC.TaskWorker.safe?({1, 1, 1})
    assert true == MC.TaskWorker.safe?({1, 2, 2})
    assert true == MC.TaskWorker.safe?({1, 3, 3})
    assert true == MC.TaskWorker.safe?({1, 3, 2})
    assert true == MC.TaskWorker.safe?({1, 3, 1})
		assert true == MC.TaskWorker.safe?({1, 3, 0})
  end
